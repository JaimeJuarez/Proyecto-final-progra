...
btnAdd.addEventListener('click', () => {
    ...
    console.log(datos);
    fetch('http://localhost:3000/ventas/producto', {
            method: 'POST',
        ...
        fetch('http://localhost:3000/ventas/producto')
            ...
            .then(json => {
                ...
                for (let j = 0; j < json.length; j++) {
                    lista += "<option value='" + json[j].id + "'>" + json[j].name + '\n' + '$' + json[j].cost + "</option>";
...
btnListar.addEventListener('click', () => {
        fetch('http://localhost:3000/ventas/producto')
            ...
            .then(json => {
                ...
                let informacion = ` 
                <h1 class="fw-bold">Lista de compra</h1>
                    <table class="text-center" style="display: flex;
                    margin-left: auto;
                    margin-right: auto;
                    width: 200px;
                }" >
                ...
                for (let i = 0; i < json.length; i++) {
                    ...
                    result.innerHTML = informacion + '</table>';
                }
            });
    })
    //button Facturar
...
btnCarrito.addEventListener('click', () => {
    ...
    let cantidad = document.getElementById('cant').value;
    fetch(`http://localhost:3000/ventas/producto/${menu}`)
        .then(res => res.json())
        .then(json => {
...
btnFactura.addEventListener('click', () => {
    ...
    fetch('http://localhost:3000/ventas/factura', {
            method: 'POST',
